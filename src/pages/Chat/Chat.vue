              ...
              <div class="shop_con">
                ...
                <div class="shop_deal">
                  <div class="shop_deal_left">
                    <span @click="updateGoodsCount(index, false)">-</span>
                    <input type="tel" value="1" v-model="goods.buy_count">
                    <span @click="updateGoodsCount(index, true)">+</span>
                  </div>
      ...
      <div id="tab_bar">
        <div class="tab-bar-left">
          <a
            ...
            @click.stop="selectedAll(isSelecteAll)"
          ></a>
          <span style="font-size: 16px;">全选</span>
          <div class="select-all">
            合计：
            <span class="total-price">{{ getAllGoodsPrice | moneyFormat(getAllGoodsPrice) }}</span>
          </div>
        </div>
...
export default {
  ...
  methods: {
    ...mapActions(["reqCartsGoods", "selectedAll"]),
    updateGoodsCount(index, isAdd) {
      if (isAdd) {
        this.cartGoods[index].buy_count++;
      } else {
        if (this.cartGoods[index].buy_count === 1) {
          this.cartGoods[index].buy_count = 1;
        } else {
          this.cartGoods[index].buy_count--;
        }
      }
  ...
  mounted() {
    this.reqCartsGoods({ id: this.userInfo.id });
  },
  updated() {
    this.selectedAll({ isSelected: true });
  },
  filters: {
    moneyFormat(money) {
